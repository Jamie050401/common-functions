...
module Xml =
    ...
    let private getValue (element : XElement) =
        ...
        | true ->
            element.Value.Replace (element.Elements () |> ("" |> Seq.fold (fun str element -> str + element.Value)), "")
        | false -> element.Value
    ...
    let private constructNode (children : Xml.Node list) (element : XElement) =
        ...
        let elementNamespacePrefix =
            element.GetPrefixOfNamespace elementNamespace.NamespaceName
    ...
    let rec private readChildren
        (nodes : Map<int, Xml.Node list>)
        (index : int)
        (parents : Map<int, XElement>)
        (elements : Map<int, XElement list>)
        : Xml.Node =
        ...
        | [] ->
            ...
            match index = 0 with
            | true -> constructNode children parent
            | false ->
                let currentNodes = nodes.[index - 1]

                let newNodes =
                    nodes |> Map.add (index - 1) (constructNode children parent :: currentNodes)
        ...
        | child :: children ->
            ...
            | true ->
                readChildren
                    nodes
                    (index + 1)
                    (parents |> Map.add (index + 1) child)
                    (newElements |> Map.add (index + 1) (child.Elements () |> List.ofSeq))
            | false ->
                let currentNodes =
                    try
                        nodes.[index]
                ...
                let newNodes =
                    nodes |> Map.add index (constructNode List.empty child :: currentNodes)
    ...
    let readFile (file : string) =
        ...
        |> fun element ->
            match element.HasElements with
            | true -> readChildren Map.empty 0 (Map [ 0, element ]) (Map [ 0, element.Elements () |> List.ofSeq ])
            | false -> constructNode List.empty element
